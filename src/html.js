const html = {
    main: (options, index, tran, icons) => `
        ...
        <div class="dplayer-video-wrap">
            ${html.video(true, options.video.pic, options.screenshot, options.preload, options.video.url, options.subtitle, options.video.corss, options.isQQBrowser)}
            ${options.logo ? `
    ...
    contextmenuList: (contextmenu, tran) => {
        ...
        // if contextmenu is null or length == 0, then display hide
        if (!contextmenu || 0 === contextmenu.length) {
            return '<div class="dplayer-menu" style="display:none"></div>';
        }
    ...
    video: (current, pic, screenshot, preload, url, subtitle, corss, isQQBrowser) => {
        // 是否启用字幕
        const enableSubtitle = subtitle && subtitle.type === 'webvtt';
        const enableCorss = screenshot || enableSubtitle;
        let corssValue = enableCorss ? 'anonymous' : null;
        corssValue = corss ? corss : corssValue;
        return '' +
            `<video class="dplayer-video ${current ? `dplayer-video-current"` : ``}" 
                ${pic ? `poster="${pic}"` : ``} webkit-playsinline playsinline 
                ${corssValue ? `crossOrigin="${corssValue}"` : ``}
                ${preload ? `preload="${preload}"` : ``} src="${url}"            
                ${isQQBrowser ? `x5-video-player-type="h5" x5-video-player-fullscreen="true" x5-video-orientation="landscape|portrait"` : ``}                
                >
